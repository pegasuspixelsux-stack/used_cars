...
// Dealership WhatsApp line, already shown elsewhere on the site
// (components/Footer.tsx) — digits only, as wa.me requires.
...
export default function VehicleInquiryForm({ car }: { car: PublicCar }) {
  ...
  const vehicleLabel = `${car.make} ${car.model} (${car.year})`;

  function buildWhatsappMessage(): string {
    const intro = `Hola! Me interesa el ${vehicleLabel} — ${formatPriceUsd(car.price)}.`;
    const who = name ? `Mi nombre es ${name}.` : "";
    const extra = message.trim();
    return [intro, who, extra].filter(Boolean).join(" ");
  }
